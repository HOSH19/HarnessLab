...
def test_read_incident_returns_fixture() -> None:
    """read_incident loads I-101 with DB-504 pool exhaustion details."""
    init_flaky_tools(None)
    payload = json.loads(read_incident.invoke({"incident_id": "I-101"}))
    assert payload["id"] == "I-101"
    assert "db-504" in payload["summary"].lower()
...
def test_correlate_timeline_for_deploy_incident() -> None:
    """I-103 timeline ties the error spike to deploy v2.14.0."""
    init_flaky_tools(None)
...
def test_search_runbooks_finds_security_guidance() -> None:
    """Runbook search returns leaked-key remediation, not deploy rollback."""
    init_flaky_tools(None)
    payload = search_runbooks.invoke({"query": "leaked API key"})
    assert "revoke" in payload.lower()
    assert "git history" in payload.lower()
